...
NEGATIVE_CONTROLS = [
    ...
    # A Markdown-escaped pipe is cell content, not a column break: `a \|\| b` in an evidence cell
    # must not shift the Result column.
    ("validate_verification", "an escaped pipe inside an evidence cell is one cell",
     {"verification.md": lambda s: s.replace(
          "`assert_equal 1, WebhookDelivery.count`", "`assert(a \\|\\| b)`")}),
    ("validate_checks", "an escaped pipe inside a coverage member cell is one cell",
     {"checks.md": lambda s: s.replace(" · `trial_will_end` C10 | - |", " · `trial_will_end` C10 (`a \\| b`) | - |", 1)}),
]
