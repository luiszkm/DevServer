...
- **Boundary:** C21-C24 closed at `96793fd` (fix round 1 for verification round 1: F1 surviving mutant -> C21; onboarding errors and scene alerts at 360 -> C22, C23; populated scenes -> C24), plus `fc4dc84` removing the `.login-panel, .onboarding { width: 100% }` override the Verifier showed to be a no-op (`.center-screen` flex already shrinks the 460px panel; plan `Problem` overstated that one). `npx vitest run` (289), `tsc --noEmit`, `eslint src e2e` and the full `npx playwright test` (90) green at `fc4dc84`, against a `:3100` build confirmed to serve the CSS without the removed rule
- **Settled mid-build:** nothing asked of the user. C22-C24 needed no layout change: all 11 new cases were green on the first run, so their value is as regression proofs; C21 was confirmed red under the F1 mutant (`if (!open)`) before being kept
- **Abandoned:** nothing
- **Boundary:** C25-C26 closed at `963c07e` (fix round 2 for verification round 2: LOJA populated state -> C25; C22 precision, error inside `.onboarding` -> C26); C24's body moved into a shared `fitsPopulated` helper and C22/C26 share `onboardingError`, assertions unchanged. `npx vitest run` (289), `tsc --noEmit`, `eslint e2e` and the full `npx playwright test` (93) green at `963c07e`
- **Settled mid-build:** nothing asked of the user. C25 and C26 were green on the first run - no layout change needed, as the Verifier's round 2 probe predicted
- **Abandoned:** nothing
