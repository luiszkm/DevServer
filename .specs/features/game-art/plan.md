...
4. out: `<img class="pixelated">` servido estático pelo Next a partir de `web/public/art`; fundos via `background-image` inline no elemento da cena (`battle-<region>` em `.battle`, `server` em `.server`; inline também no caminho fixo, que é o que o teste de tela enxerga em jsdom), com posição, tamanho ×4 (1280x720) e `pixelated` na classe CSS da cena em `globals.css`
