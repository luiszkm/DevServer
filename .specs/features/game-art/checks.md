...
- **Abandoned:** nothing
- **Boundary:** C12-C22 closed at `e91e6b6` (24 icons + `art.test.tsx` groups `be8c914`, shop/avatar gear `89b9cb7`, skills + HUD `aa6a02a`, deploy `a4c0091`, e2e `e91e6b6`); every S2 proof, `npx vitest run` (263), `tsc --noEmit`, `eslint src`, `make art-check` and the full `npx playwright test` (11) green at that commit. For S3: `art.test.tsx` has an `icons(names)` helper for 16x16 groups; no S2 test name contains `rack`, `office`, `region` or `scene background`
- **Settled mid-build:** nothing asked of the user. Builder choices: the HUD `SKILL PTS` value and its skill chips now share one `.hud-row` so 32px chips fit the 96px card; `.skill-chip` grew to 32x32 and drops its purple box when it holds an `<img>` (`:has(> img)`); `.deploy-type-name` is a flex row so the icon sits before the name; HUD icons pass `fallback=""` (no catalog glyph exists for them). Plan `Impact` corrected: `web/e2e/shop.spec.ts` and `skills.spec.ts` did assert glyphs (`{C}` in the `bebida` slot, `</>` in the HUD chip) and were updated to the C17/C19 images
- **Abandoned:** nothing
- **Boundary:** C23-C32 closed at `20d840f` (27 specs + PNGs and `art.test.tsx` groups `6169d41`, server `22f7398`, office `b957670`, world `e33a4a0`, office wall fit `20d840f`); every S3 proof, `npx vitest run` (273), `tsc --noEmit`, `eslint src`, `make art-check` (72 specs, no `WARN`) and the full `npx playwright test` (11) green at that commit. All 32 checks are closed; next step is the Verifier over the full set
- **Settled mid-build:** nothing asked of the user. Builder choices: `.server`, `.office-room` and `.world-map` take the background as an inline `backgroundImage` even though the path is fixed (jsdom never loads `globals.css`, so the inline style is what C28/C30/C31 can see), with `center / 1280px 720px no-repeat` + `image-rendering: pixelated` in the class as `.battle` does (plan `Flow` 4 updated); the map marker is a `.node-marker` box (yellow border + glow when `here`) that carries the locked `filter` inline, so the `tag` fallback greys out too, and `.node-diamond` CSS was removed; `.server-slot-glyph`, `.server-card-glyph` and `.office-detail-glyph` grew to 36px to hold ×2 art; the office background wall ends at y=50 so, cropped by a ~450x545 room, it sits behind the PAREDE row; the old `marks map nodes as here, open or locked` test now asserts `here` / no filter / locked filter instead of diamond classes; plan `Impact` gained `OfficeScene.test.tsx:108,110` and `WorldScene.test.tsx:107`; no `web/e2e` spec asserted a rack, furniture or region glyph, none changed
- **Abandoned:** dirt roads on the world map (they implied links that cannot line up with the markers once the centred background is cropped) and a rug in the office background (it duplicated the buyable `tapete`)
- **Boundary:** C33 closed at `f8c2633` (new `web/e2e/art.spec.ts`, one test per scene titled `scene art scale <selector>`); the C33 proof (4), the full `npx playwright test` (15), `npx vitest run` (273), `tsc --noEmit` and `eslint src` green at that commit. No CSS change was needed: removing `/ 1280px 720px` from `.server` and `image-rendering: pixelated` from `.battle` turned those two tests red (`auto` received), restored before the commit
- **Settled mid-build:** the user chose to commit `.claude/skills/pixel-assets/` to close verification gap 1 (commit 17001db)
- **Abandoned:** nothing
